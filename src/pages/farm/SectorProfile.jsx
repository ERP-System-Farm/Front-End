...
// Define local subcomponents similar to EnclosureDashboard for Sector context
const SectorIrrigationTab = ({ sectorId, onRowClick }) => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true)
      const res = await reportsApi.getIrrigations({ enclosure: sectorId, page })
      if (res.data.results) {
      ...
      console.error('Error fetching sector irrigation reports:', err)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-slate-800 text-lg flex items-center gap-2">
          ...
          تقارير الري والتسميد للقطاع (مجمعة)
        </h3>
      </div>
          ...
          {reports.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} className="h-32 text-center text-slate-400 font-bold">
                لا توجد تقارير ري وتسميد مسجلة في هذا القطاع
              </TableCell>
            </TableRow>
          ) : (
            reports.map(report => {
...
const SectorPestControlTab = ({ sectorId, onRowClick }) => {
  const [reports, setReports] = useState([])
  const [loading, setLoading] = useState(true)
  const [page, setPage] = useState(1)
  const [totalPages, setTotalPages] = useState(1)
  ...
  const fetchReports = async () => {
    try {
      setLoading(true)
      const res = await reportsApi.getPestControls({ enclosure: sectorId, page })
      if (res.data.results) {
        setReports(res.data.results)
        setTotalPages(Math.ceil(res.data.count / 10))
      } else {
      ...
      console.error('Error fetching sector pest control reports:', err)
    } finally {
      setLoading(false)
    }
  }
          ...
          {reports.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="h-32 text-center text-slate-400 font-bold">
                لا توجد تقارير مكافحة مسجلة في هذا القطاع
              </TableCell>
            </TableRow>
          ) : (
            reports.map(report => (
              ...
              >
                ...
                <TableCell className="font-bold">{dayjs(report.date).format('DD MMM YYYY')}</TableCell>
                <TableCell>{report.engineer_name || 'غير مسجل'}</TableCell>
                <TableCell>{report.pesticide_item_name || report.custom_pesticide_name || 'غير حدد'}</TableCell>
...
const SectorWarehouseConsumptions = ({ sectorId }) => {
  ...
  useEffect(() => {
    const fetchMovements = async () => {
      try {
        setLoading(true)
        const response = await api.get(`warehouse/movements/?location=${sectorId}`)
    ...
    <Card className="border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-slate-800 text-lg flex items-center gap-2">
          ...
          المسحوبات المخزنية للقطاع (مجمعة)
        </h3>
        <input
          type="text"
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-32 text-center">
                  <div className="flex flex-col items-center justify-center text-slate-400">
                    <p className="font-bold text-sm">لا توجد مسحوبات مطابقة للبحث في هذا القطاع</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
  ...
  useEffect(() => {
    ...
    const fetchSectorMedia = async () => {
      try {
        ...
        console.error("Failed to fetch sector media feed", err)
      } finally {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-slate-800">خطأ في الوصول إلى بيانات القطاع</h2>
    ...
    <div className="min-h-screen bg-slate-50 pb-24 relative" dir="rtl">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <h1 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">
                  {loading ? <Skeleton className="h-10 w-64" /> : `ملف قطاع: ${profile?.name}`}
                </h1>
                {!loading && (
              ...
              <p className="text-slate-500 flex items-center gap-2 text-xs sm:text-sm font-bold">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <PremiumStatCard
            icon={TreePine}
            label={isRTL ? 'إجمالي عدد أشجار القطاع' : 'Total Tree Density'}
            value={profile?.tree_count ? `${profile.tree_count.toLocaleString()} شجرة` : (isRTL ? 'بدون أشجار' : '0 Trees')}
            subtext={isRTL ? 'مجموع الأشجار الفعلي في حوشات هذا القطاع' : 'Sum of actual trees in enclosures'}
          ...
          <PremiumStatCard
            icon={Weight}
            label={isRTL ? 'معدل إنتاج القطاع الكلي' : 'Total Harvest Performance'}
            value={(() => {
          ...
          <PremiumStatCard
            icon={Droplet}
            label={isRTL ? 'حالة ري القطاع وتسميده' : 'Sector Irrigation Status'}
          ...
          <PremiumStatCard
            icon={Activity}
            label={isRTL ? 'إحصائيات جهد القطاع الكلي' : 'Total Effort Metrics'}
            ...
            <Tabs defaultValue="journal" className="w-full" dir="rtl">
              <div className="px-4 pt-4 pb-2 border-b border-slate-100 overflow-x-auto">
                <TabsList className="bg-slate-100/50 p-1 rounded-xl w-max min-w-full justify-start">
                  <TabsTrigger value="journal" className="rounded-lg px-6 font-bold data-[state=active]:bg-white data-[state=active]:text-emerald-700 data-[state=active]:shadow-sm">
                    <History className="w-4 h-4 ml-2" /> سجل عمليات القطاع
                  </TabsTrigger>
              ...
              <div className="p-6 bg-slate-50/30 rounded-b-2xl">
                ...
                <TabsContent value="warehouse" className="mt-0 outline-none">
                  <div className="mb-4">
                    <h3 className="text-lg font-bold text-slate-800">سجل استهلاك المخزون للقطاع</h3>
                    <p className="text-sm text-slate-500">المواد والأصناف التي تم صرفها أو إرجاعها في حوشات هذا القطاع.</p>
                  </div>
                  <SectorWarehouseConsumptions sectorId={id} />
                </TabsContent>

                <TabsContent value="attachments" className="mt-0 outline-none">
                  <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-4">
                    <div className="mb-4">
                      <h3 className="text-lg font-bold text-slate-800">معرض وسائط التوثيق الميداني للقطاع</h3>
                      <p className="text-sm text-slate-500">المرفقات والوسائط التي تم رفعها وتوثيقها في تقارير حوشات هذا القطاع.</p>
                    </div>
                    ...
                    {loadingAttachments ? (
                      ...
                      <div className="py-14 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-850 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 text-center">
                        ...
                        <h4 className="text-slate-705 font-bold mb-1.5">لا توجد مرفقات في هذا القطاع</h4>
                      </div>
                    )}
                  </div>
                </TabsContent>
